...
let monthsQuantity = document.querySelector('.months');
let daysQuantity = document.querySelector('.days');
let hoursQuantity = document.querySelector('.hours');
let minutesQuantity = document.querySelector('.minutes');
let secondsQuantity = document.querySelector('.seconds');
let buttonCountdown = document.querySelector('.countdown__form-button');
let inputCountdown = document.querySelector('.countdown__form-input');

let countdown = {
    updateTime() {
        let timer = setInterval(() => {
            let curDate = new Date();
            let releaseDate = new Date(2019, 8, 30, 23, 59, 59, 0);
            curDate.getMonth();
            let month = releaseDate.getMonth() - curDate.getMonth();
            let date = releaseDate.getDate() - curDate.getDate();
            let hours = releaseDate.getHours() - curDate.getHours();
            let minutes = releaseDate.getMinutes() - curDate.getMinutes();
            let seconds = releaseDate.getSeconds() - curDate.getSeconds();
            if ((month + '').length < 2) month = '0' + month;
            if ((date + '').length < 2) date = '0' + date;
            if ((hours + '').length < 2) hours = '0' + hours;
            if ((minutes + '').length < 2) minutes = '0' + minutes;
            if ((seconds + '').length < 2) seconds = '0' + seconds;
            monthsQuantity.innerText = month;
            daysQuantity.innerText = date;
            hoursQuantity.innerText = hours;
            minutesQuantity.innerText = minutes;
            secondsQuantity.innerText = seconds;
        }, 1000);
    },
    createJson() {
        let number = 1;
        buttonCountdown.addEventListener('click', () => {
            const json = JSON.stringify(inputCountdown.value);
            localStorage.setItem(`user${number++}`, json);
        })
    }
}
